...
import { saveProfile, setUserProfile } from "../../store/authSlice"; // Assuming check authSlice exports
import Modal from "../common/Modal";
import { updateProfile } from "../../api/profileApi"; // Or use thunk
...
export default function EditProfileModal({ user, isOpen, onClose }) {
    ...
    useEffect(() => {
        if (user) {
            setForm({
                ...
                profile_image: null,
                preview: user.profile_image ? `http://localhost/project_backend/${user.profile_image}` : null
            });
        }
    }, [user]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.success) {
                // Dispatch the updated user object returned from backend
    ...
    return (
        <Modal title="Modifier le profil" isOpen={isOpen} onClose={onClose}>
            <div className="modal-body">
                <div className="form-group" style={{ textAlign: 'center', marginBottom: '20px' }}>
                    <div style={{
                        ...
                        background: '#1a3c3c', margin: '0 auto 10px',
                        overflow: 'hidden', border: '3px solid #4facfe'
                    }}>
                        {form.preview ? (
                ...
                <div className="form-group">
                    <label>Email</label>
                    <input type="email" name="gmailU" value={form.gmailU} onChange={handleChange} />
                </div>
            ...
            <div className="modal-actions">
                ...
                <button className="save" onClick={handleSave}>Confirmer</button>
            </div>
        </Modal>
    );
}
